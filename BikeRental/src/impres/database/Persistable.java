...
abstract public class Persistable
{
	...
	protected Properties getSchemaInfo(final String tableName)
	{
		...
		try
		{
			...
			final ResultSet columnInfo = dbMetaData.getColumns(null, null, tableName, null);
	 ...
	 */
	//------------------------------------------------------------
	protected Vector<Properties> getPersistentState(final Properties schema, final Properties where)
	{
		...
		try
		{
			...
			final SQLSelectStatement theSQLStatement = new SQLSelectStatement(schema, where);
			...
			final Statement theStatement = theDBConnection.createStatement();
			...
			final ResultSetMetaData rsMetaData = theResultSet.getMetaData();

			numRSColumns = rsMetaData.getColumnCount();
			namesRSColumns = new Vector<String>();
			for (int cnt = 1; cnt <= numRSColumns; cnt++)
			{
				final String thisColumnName = rsMetaData.getColumnName(cnt);
				namesRSColumns.addElement(thisColumnName);
			}

			final Vector<Properties> resultSetToReturn = new Vector<Properties>();

			while (theResultSet.next() == true)
			{
				final Properties thisRow = new Properties();
				for (int cnt = 1; cnt <= numRSColumns; cnt++)
				{
					final String theColumnName = namesRSColumns.elementAt(cnt-1);
					final String theColumnValue = theResultSet.getString(cnt);
	 ...
	 */
	//------------------------------------------------------------
	protected Vector<Properties> getQueriedState(final Properties selSchema,
			final Properties projectionSchema,
			final Properties where)
			{
		int numRSColumns = 0; 			// number of columns in ResultSet
		Vector<String> namesRSColumns = null;	// names of columns in ResultSet
		...
		try
		{
			...
			// construct a SQL statement from the passed parameters
			final SQLQueryStatement theSQLStatement = new SQLQueryStatement(selSchema, projectionSchema, where);
	 ...
	 */
	//------------------------------------------------------------
	protected Vector<Properties> getQueriedStateWithExactMatches(final Properties selSchema,
			final Properties projectionSchema,
			final Properties where)
			{
		int numRSColumns = 0; 			// number of columns in ResultSet
		...
		try
		{
			...
			// construct a SQL statement from the passed parameters
			final SQLQueryStatementWithExactMatches theSQLStatement =
	 ...
	 */
	//------------------------------------------------------------
	protected Vector<Properties> getSelectQueryResult(final String sqlSelectStatement)
	{
		int numRSColumns = 0; 			// number of columns in ResultSet
		Vector<String> namesRSColumns = null;	// names of columns in ResultSet
		ResultSet theResultSet = null;			// the resultset from the SQLStatement execution
		...
		try
		{
			...
			// get the column information from the ResultSet
			final ResultSetMetaData rsMetaData = theResultSet.getMetaData();
			//			System.err.println(theResultSet);
	 ...
	 */
	//------------------------------------------------------------
	protected Integer updatePersistentState(final Properties schema, 			// the table schema
			final Properties updateValues,	// the values to update
			final Properties whereValues) 	// the where values
					throws SQLException
	{
		try
		{
			...
			// construct a SQL statement from the passed parameters
			final SQLUpdateStatement theSQLStatement = new SQLUpdateStatement(schema, updateValues, whereValues);
			// DEBUG System.out.println("SQL Statement: " + theSQLStatement.toString());
			...
			final int returnCode = theStatement.executeUpdate(theSQLStatement.toString());
	 ...
	 */
	//------------------------------------------------------------
	protected Integer insertAutoIncrementalPersistentState(final Properties schema, 		// the table schema
			final Properties insertValues)	// the values to update
					throws SQLException
	{
		...
		try
		{
			...
			// construct a SQL statement from the passed parameters
			final SQLInsertStatement theSQLStatement = new SQLInsertStatement(schema, insertValues);
			...
			// The method executeUpdate executes a query on the database. The
			// return result is of type integer which indicates the number of rows updated
			theStatement.executeUpdate(theSQLStatement.toString(), Statement.RETURN_GENERATED_KEYS);
	 ...
	 */
	//------------------------------------------------------------
	protected Integer insertPersistentState(final Properties schema, 			// the table schema
			final Properties insertValues)	// the values to update
					throws SQLException
	 ...
	 */
	//------------------------------------------------------------
	protected Integer deletePersistentState(final Properties schema,			// the table schema
			final Properties whereValues)		// the values to use to identify the delete row
					throws SQLException
	{
		try
		{
			...
			// construct a SQL statement from the passed parameters
			final SQLDeleteStatement theSQLStatement = new SQLDeleteStatement(schema, whereValues);
